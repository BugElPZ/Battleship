...
test('Game', () => {
  ...
  for (let i = 0; i < attackP1.length; i++) {
    ...
    expect(game.attack(attackP1[i][0], attackP1[i][1])).toBe(attackP1[i][2]);
  };
  expect(game.getQueue()).toEqual({player1: false, player2: true});
  expect(game.attack(0, 8)).toBe(-1);
  expect(game.getQueue()).toEqual({player1: true, player2: false});
  expect(game.attack(0, 3)).toBe(-1);
  expect(game.getQueue()).toEqual({player1: false, player2: true});
  expect(game.attack(0, 0)).toBe(0);
  expect(game.getQueue()).toEqual({player1: false, player2: true});
  expect(game.attack(1, 9)).toBe(-1);
  expect(game.getQueue()).toEqual({player1: true, player2: false});
  expect(game.attack(9, 8)).toBe(0);
  expect(game.attack(9, 9)).toBe(2);
});